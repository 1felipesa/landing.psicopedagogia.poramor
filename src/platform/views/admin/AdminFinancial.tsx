        ...
        <div className="space-y-8 animate-fadeIn px-4 md:px-0">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm md:text-base transition-colors">Gestão de faturamento e recebimentos.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-[24px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-row sm:flex-col items-center sm:items-start gap-4 sm:gap-0 transition-colors">
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-2xl flex-shrink-0">
                        <TrendingUp size={24} />
                    </div>
                    <div className="sm:mt-4">
                        <p className="text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400">A Receber (Mês)</p>
                        <h3 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white transition-colors">R$ {stats.pendingMonth.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-[24px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-row sm:flex-col items-center sm:items-start gap-4 sm:gap-0 transition-colors">
                    ...
                    <div className="sm:mt-4">
                        <p className="text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400">Caixa (Mês)</p>
                        <h3 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white transition-colors">R$ {stats.paidMonth.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-[24px] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-row sm:flex-col items-center sm:items-start gap-4 sm:gap-0 transition-colors">
                    ...
                    <div className="sm:mt-4">
                        <p className="text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400">Inadimplência</p>
                        <h3 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white transition-colors">R$ {stats.overdue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    </div>
                </div>
            </div>

            {/* Main Content */}
            <div className="bg-white dark:bg-slate-800 rounded-[28px] border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden min-h-[400px] transition-colors">
                <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex flex-col md:flex-row gap-4 items-center justify-between">
                    <h3 className="font-bold text-slate-800 dark:text-white">Histórico de Cobranças</h3>
                    <div className="relative w-full md:max-w-xs">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" size={18} />
                        <input
                            type="text"
                            placeholder="Buscar paciente ou descrição..."
                            className="w-full pl-12 pr-4 py-2.5 rounded-full bg-slate-50 dark:bg-slate-900 border-none focus:ring-2 focus:ring-primary-500 transition-all outline-none text-sm text-slate-800 dark:text-slate-200"
                            value={searchTerm}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-900/40 text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-wider">
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                            {loading ? (
                                <tr><td colSpan={6} className="text-center py-20 text-slate-400 dark:text-slate-500">Carregando dados...</td></tr>
                            ) : filteredInvoices.length === 0 ? (
                                <tr><td colSpan={6} className="text-center py-20 text-slate-400 dark:text-slate-500">Nenhuma cobrança encontrada.</td></tr>
                            ) : (
                                filteredInvoices.map(inv => (
                                    <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                                        ...
                                        <td className="px-6 md:px-8 py-5">
                                            <div className="flex items-center gap-2 md:gap-3">
                                                ...
                                                <span className="text-xs md:text-sm font-bold text-slate-800 dark:text-slate-200 truncate max-w-[100px] md:max-w-none">{inv.patient?.full_name}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 md:px-8 py-5 hidden md:table-cell">
                                            <div className="text-sm text-slate-600 dark:text-slate-400 line-clamp-1">{inv.description}</div>
                                        </td>
                                        <td className="px-6 md:px-8 py-5 text-right font-bold text-slate-900 dark:text-white text-sm md:text-base transition-colors">
                                        ...
                                        <td className="px-6 md:px-8 py-5 text-right flex items-center justify-end gap-2">
                                            {inv.status !== 'paid' && (
                                                <button
                                                    ...
                                                    className="text-[10px] font-bold text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 bg-primary-50 dark:bg-primary-900/40 px-3 py-1.5 rounded-full transition-colors"
                                                >
                                                    Confirmar
                                                </button>
                                            )}
            ...
            {showAddModal && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn border border-transparent dark:border-slate-700">
                        <div className="p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h3 className="text-2xl font-bold text-slate-800 dark:text-white">Nova Cobrança</h3>
                            ...
                            <form onSubmit={handleCreateInvoice} className="space-y-5">
                                <div>
                                    ...
                                    <select
                                        className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border-none focus:ring-2 focus:ring-primary-500 outline-none text-slate-800 dark:text-white font-medium"
                                        value={selectedPatient}
                                        onChange={e => setSelectedPatient(e.target.value)}
                                        required
                                    >
                                        <option className="dark:bg-slate-900" value="">Selecione o paciente...</option>
                                        {patients.map(p => (
                                            <option className="dark:bg-slate-900" key={p.id} value={p.id}>{p.full_name}</option>
                                        ))}
                                    </select>
                                </div>
